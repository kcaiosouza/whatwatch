...
<head>
    ...
    <script>
        ...
        $.getJSON('https://api.bigdatacloud.net/data/ip-geolocation?key=209724fb28224d21a9076a2e9f16a62b', function(data) {
            ...
            $.getJSON('https://api.themoviedb.org/3/discover/tv?api_key=c14a5e48b924e8f71f90d1fe90ccd44b&language=pt-BR&include_adult=false&primary_release_date.gte=2017-01-01&primary_release_date.lte=2022-01-15&vote_average.gte=5.5&page=' + choosedPage, function(data) {
                var listOfSeries = data;
                var choosedNumberFilm = (Math.random() * (20 - 1) + 0).toFixed(0);
                var idSerie = listOfSeries.results[choosedNumberFilm].id;
    
                $.getJSON('https://api.themoviedb.org/3/tv/'+ idSerie +'?api_key=c14a5e48b924e8f71f90d1fe90ccd44b&language='+language, function(data){
                    var choosedSerie = data;
                    ...
                    if(isMobile == true) {
                        iconShare.onclick = ()=>{
                            var shareData = {
                                title: choosedSerie.title,
                                text: 'Olha essa série que eu encontrei! Vamos assistir?',
                                url: 'https://whatwatch.live/share.html#'+idSerie
                            }
                             navigator.share(shareData)
                        };
                    }else{
                        iconShare.onclick = ()=>{navigator.clipboard.writeText("https://www.whatwatch.live/share/serie.html#"+idSerie).then(alert("Link copiado com sucesso")); };
                    }
                ...
                createShareBtn()
                    ...
                    $.getJSON('https://api.themoviedb.org/3/tv/'+idSerie+'/watch/providers?api_key=c14a5e48b924e8f71f90d1fe90ccd44b', function(data){
                    ...
                    choosedSerie.genres.forEach(function(item, index){
                    ...
                    document.getElementById('duration').innerText = choosedSerie.number_of_seasons + " Temporada(s) | " + choosedSerie.number_of_episodes + " Episodios";
    
                    // QUEM VAI MUDAR A IMAGEM DE FUNDO
                    if(choosedSerie.backdrop_path == null){
                        ...
                        document.getElementById("background").src = linkImageBG+choosedSerie.backdrop_path
                        document.getElementById("background").style.filter = "blur(1rem)"
                    }
    
                    // QUEM VAI MUDAR A CAPA
                    if(choosedSerie.poster_path == null){
                        location.reload();
                    }else{
                        document.getElementById("poster").src= linkImagePoster + choosedSerie.poster_path;
                    }
    
                    
                    // QUEM MUDA AS OUTRAS INFORMAÇÕES
                    document.getElementById("nameAndYear").innerHTML = choosedSerie.name;
                    
                    document.getElementById("rate").innerHTML = choosedSerie.vote_average;
                    var styleStar = document.createElement('style');
                    styleStar.type = 'text/css';
                    styleStar.innerHTML = '.infoTwo .checked:nth-child( n ):nth-child( -n + '+(Number(choosedSerie.vote_average).toFixed(0))+' ) {color: orange;}';
                    document.getElementsByTagName('head')[0].appendChild(styleStar);
                    
                    
                    if(choosedSerie.overview == "" && language == "pt-BR"){
                        document.getElementById("overview").innerHTML = "Em breve...";
                    }else if(choosedSerie.overview == "" && language == "es"){
                        document.getElementById("overview").innerHTML = "En breve...";
                    }else if (choosedSerie.overview == "" && language == "en"){
                        document.getElementById("overview").innerHTML = "Coming soon...";
                    }else if (choosedSerie.overview == "" && language !== "pt-BR" && "es"){
                        document.getElementById("overview").innerHTML = "Coming soon...";
                    }
                    else{
                        document.getElementById("overview").innerHTML = choosedSerie.overview;
                    }
                    setTimeout(()=>{window.onload = document.getElementById('load').style.display = 'none'; document.title = choosedSerie.name + " | WhatWatch";}, 2000)
                });
                return idSerie;
            });
        });
